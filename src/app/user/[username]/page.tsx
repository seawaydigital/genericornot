...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${user.name}'s Profile — GenericOrNot`,
    description: `${user.name} has submitted ${user.comparisons.length} comparison${user.comparisons.length !== 1 ? "s" : ""} on GenericOrNot.`,
  };
}
    ...
    <div className="mx-auto max-w-3xl px-4 md:px-6 py-10 space-y-10">
      ...
      <section className="bg-gray-900 border border-gray-800 rounded-xl p-6 flex items-center gap-5">
        {user.image ? (
          ...
          <div className="w-18 h-18 rounded-full bg-emerald-500/20 flex items-center justify-center text-2xl font-bold text-emerald-400 shrink-0 w-[72px] h-[72px]">
        ...
        <div className="ml-auto flex gap-6 text-center">
          <div>
            <p className="text-white font-bold text-lg">{user.comparisons.length}</p>
            <p className="text-gray-500 text-xs">Comparisons</p>
          </div>
          <div>
            <p className="text-white font-bold text-lg">{user.evidence.length}</p>
      ...
      <section className="space-y-4">
        ...
        {user.comparisons.length === 0 ? (
          <p className="text-gray-500 text-sm">No comparisons submitted yet.</p>
        ) : (
              ...
              return (
                ...
                >
                  <div className="min-w-0">
                    {comp.status === "APPROVED" ? (
                      <Link
                        ...
                        className="text-white font-medium hover:text-emerald-400 transition-colors text-sm leading-snug block"
                      >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {comp.status === "APPROVED" && (
                      <VerdictBadge verdict={comp.verdict} size="sm" />
                    )}
            ...
            {user.evidence.map((ev) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  ...
                  <Link
                    href={`/compare/${ev.comparison.slug}`}
                    className="text-gray-400 hover:text-emerald-400 transition-colors text-xs"
                  >
                    {ev.comparison.genericProductName} vs {ev.comparison.nameBrandProductName}
                  </Link>
                  <span className="text-gray-600 text-xs">•</span>
                  <span className="text-gray-600 text-xs">{formatDate(ev.createdAt)}</span>
                </div>
              </div>
            ))}
          </div>
